...
```json
{
  ...
  "form": "Form 3",
  "schoolCode": "NRB001",
  "teacherCode": "MWALIMU01"
}
```
`teacherCode` is optional. If provided, it must belong to a teacher at the same school (matched via `schoolCode`) and links the student to that teacher, so their sessions appear in the teacher's `/sessions/class` view. Omitting it still creates the account, but the student won't appear on any teacher's dashboard until a `teacher_id` is assigned.
