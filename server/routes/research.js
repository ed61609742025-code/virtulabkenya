...
router.post('/cpcat/submit', apiLimiter, authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Error saving CPCAT assessment:', err.message);
    res.status(500).json({ error: 'Failed to record CPCAT assessment.' });
  }
});

// GET /api/research/cpcat/status — Check pre/post test completion status for current student
router.get('/cpcat/status', authMiddleware, async (req, res) => {
  try {
    const studentId = req.user.id;
    ...
    console.error('Error fetching CPCAT status:', err.message);
    res.status(500).json({ error: 'Failed to fetch assessment status.' });
  }
});

// GET /api/research/cpcat/mine — Fetch student's assessments
router.get('/cpcat/mine', authMiddleware, async (req, res) => {
  try {
    const studentId = req.user.id;
    const assessments = await researchRepo.getStudentAssessments(studentId);
    res.json({ success: true, assessments });
  } catch (err) {
    console.error('Error fetching student assessments:', err.message);
...
router.post('/sus/submit', apiLimiter, authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Error recording SUS survey:', err.message);
    res.status(500).json({ error: 'Failed to record SUS survey.' });
  }
});

// POST /api/research/tam/submit — Record Technology Acceptance Model (TAM 3) questionnaire
router.post('/tam/submit', apiLimiter, authMiddleware, async (req, res) => {
  try {
    const userId = req.user.id;
    const userRole = req.user.role || 'student';
    const schoolId = req.user.school_id || null;
    ...
    console.error('Error recording TAM survey:', err.message);
    res.status(500).json({ error: 'Failed to record TAM survey.' });
  }
});
...
router.get('/analytics/summary', authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== 'teacher' && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Access forbidden. Teachers & Admins only.' });
    }
    ...
    console.error('Error fetching research summary:', err.message);
...
router.get('/export/csv', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Error exporting research CSV dataset:', err.message);
    res.status(500).json({ error: 'Failed to export research CSV dataset.' });
  }
});
