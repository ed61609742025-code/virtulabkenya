...
router.post('/', apiLimiter, authMiddleware, async (req, res) => {
  try {
    ...
    if (!system_id) {
      return res.status(400).json({ error: 'Reaction system ID is required.' });
    }
    ...
    console.error('Error saving energy practical session:', err.message);
    return res.status(500).json({ error: 'Could not record thermochemistry practical session.' });
  }
});
...
router.get('/mine', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Error fetching student energy sessions:', err.message);
    return res.status(500).json({ error: 'Could not fetch thermochemistry practical history.' });
  }
});
